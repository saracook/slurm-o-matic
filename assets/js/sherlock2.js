  ...
  var SlurmOMatic = {
    ...
    renderUI: function() {
      ...
      function populateModules(config) {
        ...
        var selectedModule = "";
        fetch("./includes/sh_modules.yaml")
...
return data.text;
      ...
      function populateOption(package, sessionModulesArray, category) {
        ...
        $.each(package.versions, function(index, version) {
          ...
          isDefaultVersion = version.markedDefault;
          if (isDefaultVersion) {
            isDefaultVersion = 'data-default="true"';
          } else {
            isDefaultVersion = ""
          }
          console.log('props', version.properties);
          var propertyStrings = "";
          $.each(version.properties, function(index, prop) {
            console.log('prop ' + index, prop);
            $.each(prop, function(key, value) {
              propertyStrings += `data-${key}="${value}" `;
